    ...
    for word in str_lst:
        if word[0].isdigit():
            if count_num(a_string) == 1 and str_lst[str_lst.index(word)-1] == 'at':
                if str_lst.index(word) == len(str_lst)-1:                     # for time like 12:00 or 00:00
                    time1 = word
                else:                                                         # for time like 6:30 pm, 10:00 am
            ...
            time1 = default_time
    if len(time1.split()) > 1:                                                # convert 12 hour clock into 24 hour clock
...
def notification_message(a_string):
    ...
    if ' '.join(str_lst[:3]) == 'Remind me to':                  # for reminders beginning with 'Remind me to'
        ...
        if 'at' in str_lst:
            return ' '.join(str_lst[:str_lst.index('at')])       # message will be till 'at' i.e. till time
        else:
            return a_string                                      # message will be the reminder itself


with open("C:/Users/Acer/Desktop/reminder_data.txt", "r") as data:
    rem_lst = data.readlines()
    while True:
        for rem in rem_lst:
            if notification_time(convert_time(rem)) == datetime.now().strftime('%H:%M'):
                notification.notify(title='Reminder:',
                                    message=notification_message(convert_time(rem)),
                                    app_icon="C:/Users/Acer/Desktop/unnamed.ico")
                del rem_lst[rem_lst.index(rem)]
